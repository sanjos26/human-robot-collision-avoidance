...
predictor = load_model('OcclusionHandler_conv_32dim_OcclON.keras', custom_objects={"mse": tf.keras.losses.MeanSquaredError()})
...
class PosePredictor(Node):
    ...
    def predict_pose(self, inp_set):
        ...
        torso_scale = np.linalg.norm(inp_set[1]-inp_set[8])
        l3 = inp_set[8]
        input=inp_set
        inp_set = inp_set-l3
        ...
        past = np.delete(inp_set,8,axis=0)/torso_scale
        ...
        f_pose = np.insert(f_pose,8,l3,axis=0)
        outp_final = f_pose
        
        # for i in range(outp_final.shape[0]):
        #     if input[i][2]!=0:
        #         outp_final[i]=input[i]
        ...
        curr_markers = self.publish_markers(input, 0, (0.0, 1.0, 0.0))   # Green
        # pred_markers = self.publish_markers(outp_final, 100, (1.0, 0.0, 0.0))  # Red

        # Add bone links
        curr_links = self.publish_links(input, 200, (0.0, 1.0, 0.0))
